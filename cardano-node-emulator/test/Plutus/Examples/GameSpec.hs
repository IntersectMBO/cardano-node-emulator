  ...
  arbitraryAction s =
    ...
    where
      ...
      genLockAction = do
        ...
        pure (Lock w)
          <*> genGuess
          <*> QC.choose (Ada.getLovelace Ledger.minAdaTxOutEstimated, Ada.getLovelace (Ada.adaOf 100))
...
instance RunModel GameModel E.EmulatorM where
  ...
  perform _ cmd _ = case cmd of
    Lock w secret val -> do
      ...
      lift $
        submitLockTx
          (E.knownAddresses !! wIx)
          (E.knownPaymentPrivateKeys !! wIx)
          LockArgs
            { lockArgsGameParam = gameParamTest
    ...
    Guess w secret -> do
      ...
      lift $
        submitGuessTx
          ...
          (E.knownPaymentPrivateKeys !! wIx)
          GuessArgs
            { guessArgsGameParam = gameParamTest
