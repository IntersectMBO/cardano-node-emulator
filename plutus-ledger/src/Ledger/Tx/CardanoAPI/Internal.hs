...
instance FromJSON (C.PlutusScript C.PlutusScriptV1) where
  parseJSON =
    Aeson.withText "PlutusScript PlutusScriptV1" $
...
instance FromJSON (C.PlutusScript C.PlutusScriptV2) where
  parseJSON =
    Aeson.withText "PlutusScript PlutusScriptV2" $
      either (error "instance FromJSON PlutusScript: deserialisation failed") pure
...
instance ToJSON C.HashableScriptData where
  toJSON = toJSON . C.toPlutusData . C.getScriptData
instance FromJSON C.HashableScriptData where
  parseJSON = fmap (C.unsafeHashableScriptData . C.fromPlutusData) . parseJSON
  ...
  toJSON (C.PlutusScriptWitness _ version script datum red exUnits) =
    Aeson.object
      ...
      , case version of
          ...
          C.PlutusScriptV3 -> error "ScriptWitness: Plutus V3 not supported in Babbage era"
      , "datum" .= datum
      , "redeemer" .= red
  ...
  parseJSON = Aeson.withObject "ScriptWitness" $ \v ->
    ...
    where
      mkPSW
        :: Aeson.Object
        -> Parser
            (C.ScriptDatum ctx -> C.ScriptRedeemer -> C.ExecutionUnits -> C.ScriptWitness ctx C.BabbageEra)
      mkPSW v = do
        C.AnyPlutusScriptVersion version <- v .: "version"
        case version of
          C.PlutusScriptV1 ->
            C.PlutusScriptWitness C.PlutusScriptV1InBabbage C.PlutusScriptV1 <$> v .: "scriptOrReferenceInput"
          C.PlutusScriptV2 ->
            C.PlutusScriptWitness C.PlutusScriptV2InBabbage C.PlutusScriptV2 <$> v .: "scriptOrReferenceInput"
          C.PlutusScriptV3 -> error "ScriptWitness: Plutus V3 not supported in Babbage era"
...
instance FromJSON (C.TxMintValue C.BuildTx C.BabbageEra) where
  ...
  parseJSON (Aeson.Object v) =
    C.TxMintValue C.MultiAssetInBabbageEra
      <$> v .: "value"
      <*> v .: "policyMap"
  ...
  parseJSON (Aeson.Array v) = C.TxExtraKeyWitnesses C.ExtraKeyWitnessesInBabbageEra <$> traverse parseHash (Vector.toList v)
    where
      parseHash =
        Aeson.withText
          "TxExtraKeyWitnesses"
          ( either (error "instance FromJSON (TxExtraKeyWitnesses BabbageEra): deserialisation failed") pure
...
instance FromJSON (C.TxScriptValidity C.BabbageEra) where
  ...
  parseJSON (Aeson.Bool v) =
    pure $
      C.TxScriptValidity C.TxScriptValiditySupportedInBabbageEra $
        if v then C.ScriptValid else C.ScriptInvalid
...
instance FromJSON (C.TxMetadataInEra C.BabbageEra) where
  ...
  parseJSON v =
    either (parseFail . show) (pure . C.TxMetadataInEra C.TxMetadataInBabbageEra) $
      C.metadataFromJson C.TxMetadataJsonDetailedSchema v

instance ToJSON (C.ScriptInEra C.BabbageEra) where
  toJSON (C.ScriptInEra _ (C.SimpleScript _)) = error "ToJSON ScriptInEra: Simple scripts not supported"
  toJSON (C.ScriptInEra _ (C.PlutusScript version script)) =
    Aeson.Object
      [ "version" .= C.AnyPlutusScriptVersion version
      , case version of
          C.PlutusScriptV1 -> "script" .= script
          C.PlutusScriptV2 -> "script" .= script
          C.PlutusScriptV3 -> error "ScriptInEra: Plutus V3 not supported in Babbage era"
      ]
...
instance FromJSON (C.TxCertificates C.BuildTx C.BabbageEra) where
  parseJSON _ = pure C.TxCertificatesNone
