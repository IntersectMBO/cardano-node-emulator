...
findTxInByTxOutRef :: PV3.TxOutRef -> PV3.TxInfo -> Maybe PV3.TxInInfo
findTxInByTxOutRef outRef PV3.TxInfo{PV3.txInfoInputs} =
  find (\PV3.TxInInfo{PV3.txInInfoOutRef} -> txInInfoOutRef == outRef) txInfoInputs
...
-- | Find the input currently being validated. (Temporarily copied until cardano-ledger updates its plutus-core deps to >= 1.19.)
findOwnInput :: ScriptContext -> Maybe PV3.TxInInfo
findOwnInput
  ScriptContext
    ...
    find
      (\PV3.TxInInfo{PV3.txInInfoOutRef} -> txInInfoOutRef == txOutRef)
      txInfoInputs
