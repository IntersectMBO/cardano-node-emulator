module Cardano.Node.Emulator.Test.Coverage (
  getCoverageData,
  writeCoverageReport,
) where
...
import Ledger.Index (ValidationResult, getEvaluationLogs)
...
getCoverageData :: ValidationResult -> CoverageData
getCoverageData = foldMap (coverageDataFromLogMsg . Text.unpack) . getEvaluationLogs
...
includeNearby swipes = excluding 1 swipes
  where
    ...
    excluding n nSwipes
      | n > nextSwiped =
          error . unlines $ ("Bad excluding: " ++ show n) : map show (take 10 nSwipes)
    ...
    including n ((next, swipe) : nSwipes)
      | n > next =
          error . unlines $ ("Bad including: " ++ show n) : map show (take 10 ((next, swipe) : nSwipes))
      | n == next =
...
fillSmallGaps ((n, swipes) : (n', swipes') : nSwipes)
  | n + 4 >= n' =
      (n, swipes) : [(i, []) | i <- [n + 1 .. n' - 1]] ++ fillSmallGaps ((n', swipes') : nSwipes)
...
encode = Text.unpack . text . Text.pack
...
coverageReportHtml fs = element "body" [] $ report
  where
    report =
      header
        ++ concat ["<hr>" ++ file name body covx annots covi | FileInfo name body covx annots covi <- fs]
    header =
      element "h1" [] "Files"
        ++ element
          "ul"
...
swipeLine c line s@(Swipe (_, from) (_, to) stat : swipes)
  | c < from =
      element "span" [("style", offChainStyle)] (encode $ take (from - c) line)
